...
namespace EndpointPilotJsonEditor.App.ViewModels
{
    ...
    public class RegOpsEditorViewModel : OperationEditorViewModelBase<RegOperation>
    {
        ...
        /// <summary>
        /// Signs and saves the registry operations
        /// </summary>
                ...
                if (dialogResult == true && certificateDialog.DataContext is CertificateSelectionViewModel viewModel)
                {
                    ...
                    if (selectedCertificate != null)
                    {
                        ...
                        if (IsValid)
                        {
                            ...
                            await _jsonFileService.WriteRegOperationsAsync(Operations.ToList(), "REG-OPS.json", selectedCertificate);
                            IsModified = false;
                            OnStatusChanged($"Registry operations digitally signed and saved successfully using certificate: {selectedCertificate.Subject}", false);
                        ...
                        {
                            OnStatusChanged("Cannot sign and save: Registry operations contain validation errors", true);
                        }
                    }
                }
                else
