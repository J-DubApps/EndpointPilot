...
<#
.SYNOPSIS
    Updater for EndpointPilot Core Scripts
.DESCRIPTION
    Downloads the latest EndpointPilot source from GitHub and updates
    the core script files in %PROGRAMDATA%\EndpointPilot\, preserving specific configuration files.
    Requires Administrator privileges to write to %PROGRAMDATA%. Does NOT update the JsonEditorTool.
.NOTES
    Author: Julian West
    Version: 1.0.0 (Update - Non-Admin)
...
$LogFile = Join-Path -Path $env:windir -ChildPath "Temp\EPilot-Update-$env:computername.log" # Updated log file name
...
try {
    ...
    Write-Host "Ensuring installation directory exists..." -ForegroundColor Cyan
    WriteLog "Ensuring installation directory exists..."
    ...
    $selfInstallerName = "Install-EndpointPilot.ps1" # Original installer name
    $selfUpdaterName = "Update-EndpointPilot.ps1"   # This script's name
    ...
    # Update summary
    Write-Host "`nEndpointPilot Core Scripts update complete!" -ForegroundColor Green
    WriteLog "`nEndpointPilot Core Scripts update complete!"
...
<#
.SYNOPSIS
    Uninstaller for EndpointPilot Core Scripts (after update)
.DESCRIPTION
    Removes updated EndpointPilot Core Scripts components from the system.
...
Write-Host "EndpointPilot Core Scripts (updated version) have been uninstalled." -ForegroundColor Green
"@
...
Write-Host "`nThank you for updating EndpointPilot Core Scripts!" -ForegroundColor Cyan
WriteLog "`nThank you for updating EndpointPilot Core Scripts!"